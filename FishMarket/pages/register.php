...
.tab-active-bg{position:absolute;left:4px;top:4px;width:calc(50% - 4px);height:calc(100% - 8px);background:linear-gradient(135deg,var(--brand-cyan),var(--brand-dark));border-radius:25px;z-index:1;will-change:transform;transform:translate3d(100%,0,0);transition:transform .4s cubic-bezier(.25,1,.5,1)}
.auth-slider-view{width:100%;flex:1;min-height:0;overflow:hidden}
.auth-slider-wrapper{display:flex;width:200%;min-height:100%;will-change:transform;transform:translate3d(-50%,0,0);transition:transform .5s cubic-bezier(.25,1,.5,1)}
...
<div class="auth-card-static">
    <div class="auth-tabs">
      ...
      <button class="auth-tab" onclick="switchTo('login')">Sign In</button>
      <button class="auth-tab active" onclick="switchTo('register')">Register</button>
    </div>
...
sliderWrapper.style.transform="translate3d(-50%,0,0)";tabBg.style.transform="translate3d(100%,0,0)";

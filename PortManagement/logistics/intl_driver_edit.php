...
$id = (int)($_GET['id'] ?? 0);
if (!$id) { redirect('/logistics/export_driver/'); exit; }

$targetDriver = null;
try { $targetDriver = dbGetRow("SELECT d.*, p.name as port_name FROM export_driver d LEFT JOIN ports p ON p.id = d.port_id WHERE d.id = ?", [$id]); } catch (Exception $e) {}
if (!$targetDriver) {
    redirect('/logistics/export_driver/');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name) {
        try {
            dbExecute("UPDATE export_driver SET name = ?, phone = ?, port_id = ?, license_no = ?, vehicle_no = ?, identification_no = ?, is_active = ? WHERE id = ?",
                [$name, $phone, $port_id, $license_no, $vehicle_no, $identification_no, $is_active, $id]);
            $message = 'International driver updated successfully.';
            $targetDriver = dbGetRow("SELECT d.*, p.name as port_name FROM export_driver d LEFT JOIN ports p ON p.id = d.port_id WHERE d.id = ?", [$id]);
...
$title = 'Edit International Driver';
  ...
  <div class="dash-content">
    ...
    <form method="post" style="max-width:820px;margin:0 auto;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
        <div>
          <h2 style="margin:0;font-size:1.3rem;">✏️ Edit International Driver</h2>

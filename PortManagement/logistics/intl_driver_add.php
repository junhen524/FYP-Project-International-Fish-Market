...
$message = ''; $error = '';
$ports = [];
try { $ports = dbGetAll("SELECT DISTINCT port_id as id, port_name as name FROM port_user WHERE port_id IS NOT NULL ORDER BY port_id"); } catch (Exception $e) {}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name) {
        try {
            dbExecute("INSERT INTO export_driver (name, phone, port_id, license_no, vehicle_no, identification_no) VALUES (?, ?, ?, ?, ?, ?)",
                [$name, $phone, $port_id, $license_no, $vehicle_no, $identification_no]);
            $newId = dbLastInsertId();
            $message = 'International driver added successfully.';
            if ($newId) {
                redirect('/logistics/export_driver/edit/?id=' . $newId);
                exit;
            }
...
$title = 'Add International Driver';
  ...
  <div class="dash-content">
    ...
    <form method="post" style="max-width:820px;margin:0 auto;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
        <div>
          <h2 style="margin:0;font-size:1.3rem;">➕ Add International Driver</h2>
          <p class="subtle" style="margin:4px 0 0;">Export Shipping Driver</p>
        </div>
        <a href="/logistics/export_driver/" class="btn btn-ghost" style="padding:8px 16px;">← Back to Intl. Drivers</a>
      </div>
          ...
          <div>
            ...
            <div class="form-grid">
              <div class="form-group">
                ...
                <input type="text" id="name" name="name" class="form-input" required placeholder="e.g. Tan Wei Ming">
              </div>
              <div class="form-group">
                <label for="phone">Phone</label>
                <input type="text" id="phone" name="phone" class="form-input" placeholder="e.g. +60 16-789 0123">
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
                <label for="port_id">Departure Port</label>
              ...
              <div class="form-group">
                ...
                <input type="text" id="vehicle_no" name="vehicle_no" class="form-input" placeholder="e.g. WXX 5678">
              </div>
              <div class="form-group">
                <label for="license_no">License No</label>
                <input type="text" id="license_no" name="license_no" class="form-input" placeholder="e.g. IDL-2024-1234">
              </div>
            </div>
          </div>

          <div class="action-buttons">
            <a href="/logistics/export_driver/" class="btn-cancel">Cancel</a>

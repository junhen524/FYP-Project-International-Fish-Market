...
$id = (int)($_GET['id'] ?? 0);
if (!$id) { redirect('/dashboard/analytics/international/users/'); exit; }

$targetUser = null;
try { $targetUser = dbGetRow("SELECT id, username, email, full_name, phone, country_code, role, account_status, created_at FROM export_user WHERE id = ?", [$id]); } catch (Exception $e) {}
if (!$targetUser) {
    redirect('/dashboard/analytics/international/users/');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete') {
    try {
        dbExecute("DELETE FROM export_user WHERE id = ?", [$id]);
        $_SESSION['_user_msg'] = 'User deleted.';
        redirect('/dashboard/analytics/international/users/');
        exit;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') !== 'delete') {
    ...
    if ($full_name && $email) {
        try {
            dbExecute("UPDATE export_user SET full_name = ?, email = ?, country_code = ?, account_status = ? WHERE id = ?",
                [$full_name, $email, $country_code, $account_status, $id]);
            $message = 'User updated successfully.';
                    $_SESSION['_user_msg'] = $message;
                    redirect('/dashboard/analytics/international/users/');
                    exit;
            $targetUser = dbGetRow("SELECT id, username, email, full_name, company_name, business_type, country_code, verification_level, account_status, identification_no FROM export_user WHERE id = ?", [$id]);
  ...
  <div class="dash-content">
    ...
    <form method="post" style="max-width:820px;margin:0 auto;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
        <div>
          <h2 style="margin:0;font-size:1.3rem;">✏️ Edit International User</h2>
          <p class="subtle" style="margin:4px 0 0;">Export Partner Account</p>
        </div>
    ...
    <form method="POST" onsubmit="return confirm('Are you sure you want to delete this user? This cannot be undone.');" style="max-width:820px;margin:20px auto 0;padding-top:20px;border-top:1px solid rgba(255,255,255,0.08);">
      <input type="hidden" name="action" value="delete">
      <button type="submit" style="width:100%;padding:12px;border:none;border-radius:10px;background:rgba(239,68,68,0.15);color:#ef4444;font-size:14px;font-weight:800;cursor:pointer;border:1px solid rgba(239,68,68,0.3);">🗑️ Delete User</button>
    </form>
  </div>
</div>
